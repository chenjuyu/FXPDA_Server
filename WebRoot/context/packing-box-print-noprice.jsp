...
<%
	...
	if(null == map){
		request.getRequestDispatcher("/common.do?packingBoxPrintNoPrice").forward(request, response);
		return;
	}
		...
		<div class="datas">
			<table class="t_s" align="center">
				<tr>
					...
					<td width="33.3%" class="center">客户:&nbsp;${datas.customer}</td>
					
					<td width="33.3%" class="center">日期:&nbsp;<fmt:formatDate value="${datas.date}" pattern="yyyy-MM-dd" /></td>
...
<!-- 					<th>零售金额</th> -->
				...
				<c:forEach begin="0" end="${fn:length(datas.details)}" items="${datas.details}" var="d">
					<tr>
						...
						<td class="center"><span class="_span">${d.quantity}</span></td>
<%-- 						<td class="right"><fmt:formatNumber type="number" value="${d.retailAmount}" pattern="0.00" maxFractionDigits="2"/></td> --%>
					...
					<tr>
						...
						<td></td>
						<td></td>
						<td align="center"><span class="_span">${datas.quantitySum}</span></td>
