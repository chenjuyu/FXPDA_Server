...
<%
	...
	if(null == map){
		request.getRequestDispatcher("/common.do?packingBoxPrintHasPrice").forward(request, response);
		return;
	}
		...
		<div class="datas">
			<table class="t_s" align="center">
				<tr>
					...
					<td width="33.3%">日期:&nbsp;<fmt:formatDate value="${datas.date}" pattern="yyyy-MM-dd" /></td>
			...
			<table class="t_d" border="1" cellspacing="0" cellpadding="0">
				...
				<c:forEach begin="0" end="${fn:length(datas.details)}" items="${datas.details}" var="d">
					<tr>
						...
						<td class="right"><span class="_span"><fmt:formatNumber type="number" value="${d.retailAmount}" pattern="0" maxFractionDigits="0"/></span></td>
					</tr>
				</c:forEach>
					<tr>
						<td class="center">合计/${datas.boxCount}箱</td>
						<td></td>
						<td></td>
						<td  class="center">${datas.quantitySum}</td>
						<td></td>
						<td class="right"><span class="_span"><fmt:formatNumber type="number" value="${datas.retailAmountSum}" pattern="0" maxFractionDigits="0"/></span></td>
					</tr>
			</table>
		</div>
		<div class="bottom" id="print_bottom">
			<table class="t_b" align="center">
				<tr>
					...
					<td width="33.3%" class="center">收货人:&nbsp;</td>
					
				</tr>
				<tr>
					<td width="33.3%">备注:&nbsp;${datas.memo}</td>
