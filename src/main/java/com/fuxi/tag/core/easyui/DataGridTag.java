...
import com.fuxi.system.util.StringUtil;
import com.fuxi.system.util.SysLogger;
import com.fuxi.tag.vo.easyui.ColumnValue;
import com.fuxi.tag.vo.easyui.DataGridColumn;
import com.fuxi.tag.vo.easyui.DataGridUrl;
import com.fuxi.tag.vo.easyui.OptTypeDirection;
...
public class DataGridTag extends TagSupport {
     ...
     * @param value
     */
    public void setColumn(String title, String field, Integer width, String rowspan, String colspan, String align, boolean sortable, boolean checkbox, String formatter, boolean hidden, String replace, String treefield, boolean image, String imageSize, boolean query, String url, String funname,
            String arg, String queryMode, String dictionary, boolean frozenColumn, String extend, String style, String downloadName, boolean isAuto, String extendParams, boolean showcheck, String valuereplace, boolean showrgb) {
        ...
        dataGridColumn.setShowcheck(showcheck);
        dataGridColumn.setValuereplace(valuereplace);
        dataGridColumn.setShowrgb(showrgb);
    ...
    public StringBuffer datatables() {
        ...
        sb.append("\"oLanguage\" : {" + "\"sLengthMenu\" : \" _MENU_ 条记录\"," + "\"sZeroRecords\" : \"没有检索到数据\"," + "\"sInfo\" : \"第 _START_ 至 _END_ 条数据 共 _TOTAL_ 条\"," + "\"sInfoEmtpy\" : \"没有数据\"," + "\"sProcessing\" : \"正在加载数据...\"," + "\"sSearch\" : \"搜索\"," + "\"oPaginate\" : {"
                + "\"sFirst\" : \"首页\"," + "\"sPrevious\" : \"前页\", " + "\"sNext\" : \"后页\"," + "\"sLast\" : \"尾页\"" + "}" + "},"); // 汉化
    ...
    public StringBuffer end() {
        ...
        sb.append("<script type=\"text/javascript\">");
        sb.append(" var page;var pageSize; ");
        if (remPage) {
            sb.append("  page = $(window.parent.document).find('.tabs-selected').attr('page'); ");
            sb.append("  pageSize = $(window.parent.document).find('.tabs-selected').attr('pageSize'); ");

        }

        sb.append(" if(page == undefined){page=1;} ");
        sb.append(" if(pageSize == undefined){pageSize=20;} ");
        ...
        if (treegrid) {
            ...
            sb.append("pageNumber:parseInt(page), ");
        }
        ...
        sb.append("pageSize: parseInt(pageSize),");
        ...
        // 设置页数;
        if (remPage) {
            sb.append("$(window.parent.document).find('.tabs-selected').attr('page',$(\'#" + name + "\').datagrid('getPager').data('pagination').options.pageNumber); ");
            sb.append("$(window.parent.document).find('.tabs-selected').attr('pageSize',$(\'#" + name + "\').datagrid('getPager').data('pagination').options.pageSize); ");
        ...
        sb.append("function get" + name + "Selections(field){" + "var ids = [];" + "var rows = $(\'#" + name + "\')." + grid + "(\'getSelections\');" + " var temp='';  for(var i=0;i<rows.length;i++){");
        if (treegrid) {
            // 地区修改
            sb.append(" if(field=='text'){ ");
            sb.append(" temp+=rows[i]['text'] ;");
            sb.append(" var t= $(\'#\'+gridname)." + grid + "('getParent',rows[i]['id']); ");
            sb.append(" while(t!=null ){");
            sb.append(" var tempstr= t['text']; ");
            sb.append(" temp=tempstr+temp;");
            // sb.append(" temp+=t['text'] ;");
            sb.append("  t= $(\'#\'+gridname)." + grid + "('getParent',t['id']); ");
            sb.append("}");
            sb.append(" ids.push(temp); ");
            sb.append("}else{ids.push(rows[i][field]);}");
        } else {
            sb.append(" ids.push(rows[i][field]); ");
        }

        sb.append("} ids.join(\',\');" + "return ids" + "};");
        sb.append("function getSelectRows(){");
        sb.append("	return $(\'#" + name + "\').datagrid('getChecked');");
        sb.append("}");


        sb.append("function searchParam(params){");
        sb.append("$(\'#" + name + "\')." + grid + "({url:'" + actionUrl + "&field=" + searchFields + "'+params});" + "}");

        if (columnList.size() > 0) {
            sb.append("function " + name + "search(){");
            sb.append(" var findsql = findParam(); ");
            sb.append("$(\'#" + name + "\')." + grid + "({url:'" + actionUrl + "&findsql='+findsql+'&field=" + searchFields + "',pageNumber:1});" + "}");
                ...
                for (DataGridColumn col : columnList) {
                    if (col.isQuery()) {
                        if (index % 3 == 0 && index != 0)
                            sb.append("<br>");
                        sb.append("<span style=\"margin-top:1px;display:-moz-inline-box;display:inline-block;\">");
        ...
        if (toolBarList.size() == 0 && !hasQueryColum(columnList)) {
            ...
            sb.append("<div style=\"height:25px;\" class=\"datagrid-toolbar\">");
        }
        sb.append("<span style=\"float:" + buttonAlign + ";margin-right:10px;\" >");
        if (toolBarList.size() > 0) {
        ...
        if (index != -1) {
            String actionurl = actionUrl.substring(0, index);
            sb.append("<input type=\"hidden\"   id=\"actionurl\" name=\"actionurl\" value=\"");
            sb.append(actionurl);
            sb.append("\"/>");
        }
        return sb;
    ...
    protected void getOptUrl(StringBuffer sb) {
        ...
        for (DataGridUrl dataGridUrl : list) {
            ...
            if (OptTypeDirection.Confirm.equals(dataGridUrl.getType())) {
                sb.append("href+=\"[<a href=\'#\' onclick=createdialog_old('提示',\'" + dataGridUrl.getMessage() + "\',\'" + url + "\',\'" + name + "\')> \";");
            }
            ...
            if (OptTypeDirection.OpenCreate.equals(dataGridUrl.getType())) {
                sb.append("href+=\"[<a href=\'#\' onclick=openEditWindow('" + dataGridUrl.getTitle() + "','" + url + "','" + name + "'," + dataGridUrl.getWidth() + "," + dataGridUrl.getHeight() + ")>\";");
            }
    ...
    protected void getField(StringBuffer sb, int frozen) {
        ...
        for (DataGridColumn column : columnList) {
            ...
            if ((column.isFrozenColumn() && frozen == 0) || (!column.isFrozenColumn() && frozen == 1)) {
                ...
                if (column.isShowrgb()) {
                    sb.append(",formatter:function(value,rec,index){");
                    sb.append(" return '<div style=\"height:27px;width:80px;background-color:#'+value+'\" ><div>';} ");
                }
                ...
                if (column.getImageSize() != null) {
                    ...
                    sb.append(",formatter:function(value,rec,index){");
                    sb.append("if(value == '' ){ ");
                    sb.append(" return '<img width=\"" + tld[0] + "\" height=\"" + tld[1] + "\" border=\"0\" src=\"plug-in/image/nopic.png\"/>';");
                    sb.append("}");
                ...
                // 暂时隐藏， 图片问题
                /*
                 * //值替换 sb.append(",formatter:function(value,rec,index){");
                 * sb.append(" var regx= '\"null\"_'; ");
                 * sb.append("  var valuelist= regx.split(','); ");
                 * sb.append("  for (var i=0;i<valuelist.length;i++){");
                 * sb.append(" var values= valuelist[i]; ");
                 * sb.append(" var lables= values.split('_'); ");
                 * sb.append(" if(lables[0]==value){"); sb.append(" return lables[1];  ");
                 * sb.append("}"); sb.append("}"); sb.append(" return  value; }");
                 */
                ...
                // 是否是checkbox字段
                if (column.isShowcheck()) {
                    sb.append(",formatter:function(value,rec,index){");
                    sb.append(" if(value=='1'){ ");
                    sb.append(" return ' <input type=\"checkbox\" disabled=\"disabled\" value=\"1\"  checked=\"checked\" name=\"" + column.getField() + "\" /> ' }");
                    sb.append(" else { ");
                    sb.append(" return ' <input type=\"checkbox\"  disabled=\"disabled\"  name=\"" + column.getField() + "\"/>  ' }}");
                }
    ...
    protected void searchboxFun(StringBuffer sb, String grid) {
        ...
        sb.append("var queryParam=$(\'#" + name + "\').datagrid('options').queryParams;");
        sb.append("queryParam[name]=value;queryParam.searchfield=name;");
        sb.append("$(\'#" + name + "\')." + grid + "({url:'" + actionUrl + "&field=" + searchFields + "',queryParams:queryParam,pageNumber:1});}");
    ...
    private String getAutoSpan(String filed, String extend) {
        ...
        nsb.append("$(document).ready(function() {").append("$(\"#" + getEntityName() + "_" + id + "\").autocomplete(\"commonController.do?getAutoList\",{").append("max: 5,minChars: 2,width: 200,scrollHeight: 100,matchContains: true,autoFill: false,extraParams:{")
        ...
        nsb.append("$(\"#" + getEntityName() + "_" + id + "\").val(row['" + filed + "']);}); });").append("</script>").append("<input type=\"text\" id=\"" + getEntityName() + "_" + id + "\" name=\"" + filed + "\" datatype=\"*\" " + extend + " nullmsg=\"\" errormsg=\"输入错误\"/>");
        return nsb.toString();
    }
    ...
    public void setRemPage(Boolean remPage) {
