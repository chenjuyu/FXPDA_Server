...
public class ChooseTag extends TagSupport {
    ...
    protected String beforefun;
    protected String optionname;
    protected String optiondialog;

    public void setOptiondialog(String optiondialog) {
    ...
    public StringBuffer end() {
        ...
        sb.append("<a href=\"#\"  style=\"height:20px\" class=\"easyui-linkbutton\"  plain=\"true\" icon=\"" + icon + "\" onClick=\"choose_" + methodname + "()\">选择</a>");
        if (isclear && StringUtil.isNotEmpty(textname)) {
            sb.append("<a href=\"#\" style=\"height:20px\"   class=\"easyui-linkbutton\" plain=\"true\" icon=\"icon-redo\" onClick=\"clearAll_" + methodname + "();\">清空</a>");
        }

        sb.append("<script type=\"text/javascript\">");
        /*
         * sb.append(" if(frameElement!=null){");
         * sb.append("var windowapi = frameElement.api, W = windowapi.opener;");
         * sb.append("}");商品资料选择策略报错
         */
        sb.append("function choose_" + methodname + "(){");

        if (beforefun != null) {
            sb.append("  var  url= ").append(beforefun).append("('").append(url).append("');");
        } else {
            sb.append(" var url = '").append(url).append("';");
        }

        sb.append("  if(!url){return false;} ");
