...
/**
 * Title: SalesOrderImpl Description: 销售订单业务逻辑接口实现类
 * 
 * @author LJ,LYJ
 * 
 */
...
public class SalesOrderImpl implements SalesOrderService {
    ...
    @Override
    public synchronized String saveSalesOrder(List<Map<String, Object>> dataList, String salesOrderID, String customerid, String departmentid, String employeeId, String businessDeptId, String memo, String type, String typeEName, String brandId, String discountRateSumStr, String lastARAmountStr,
            String preReceivalAmountStr, String privilegeAmountStr, String paymentTypeIdStr, Client client) throws Exception {
        String warehouseId = null;
        String result = String.valueOf(commonDao.getData(" select departmentId from department where SettleCustID is not null and SettleCustID <> '' and SettleCustID = ? ", customerid));
        ...
        String No = null;
        String deptCode = client.getDeptCode();
        // 判断salesOrderID是否为空，做新增主表
        if ("".equals(salesOrderID)) {
            // 生成ID
            salesOrderID = commonDao.getNewIDValue(28);
            // 生成No
            String deptType = client.getDeptType();
            if (deptType != null && ("直营店".equals(deptType) || "加盟店".equals(deptType))) {
                No = commonDao.getNewNOValue(28, salesOrderID, deptCode);
            } else {
                No = commonDao.getNewNOValue(28, salesOrderID);
            }
            if (salesOrderID == null || No == null) {
                throw new BusinessException("生成主键/单号失败");
            }
            StringBuilder insertMaster = new StringBuilder();
            insertMaster.append("insert into SalesOrder(SalesOrderID, No,Date,DeliveryDate ").append(" , CustomerID,DepartmentID,WarehouseID,EmployeeID, QuantitySum, ").append("AmountSum, SoldQuantitySum, MadeBy, MadeByDate, Type, memo,BusinessDeptID, brandId )").append(" values('")
                    .append(salesOrderID).append("', '").append(No).append("', '" + DataUtils.str2Timestamp(DataUtils.formatDate()) + "', getdate(), '").append(customerid).append("',").append(warehouseId).append(", '" + departmentid + "','").append(employeeId).append("', ").append("0").append(", ")
                    .append("0").append(", null").append(", '").append(client.getUserName()).append("', getdate() ").append(",'").append(type).append("', '").append(memo).append("','").append(businessDeptId).append("','").append(brandId).append("')");
        ...
        // 预收金额
        BigDecimal preReceivalAmount = null;
        if (null == preReceivalAmountStr || "".equals(preReceivalAmountStr) || "null".equalsIgnoreCase(preReceivalAmountStr)) {
            preReceivalAmount = null;
        } else {
            preReceivalAmount = new BigDecimal(preReceivalAmountStr);
        }
        // 应收金额
        ...
        // 获取最大的下标值
        int maxIndexNo = getMaxIndexNo(salesOrderID);
        // 循环查询或更新
        ...
        for (int i = 0; i < dataList.size(); i++) {
            ...
            if (null != boxQty && 0 != boxQty) {
                obj = commonDao.getData(" select count(1) from salesOrderdetailtemp where goodsId = '" + goodsId + "' and colorid  = '" + colorId + "' and salesOrderID = '" + salesOrderID + "' and sizeStr = '" + sizeStr + "' ");
            } else {
                obj = commonDao.getData(" select count(1) from salesOrderdetailtemp where goodsId = '" + goodsId + "' and colorid  = '" + colorId + "' and salesOrderID = '" + salesOrderID + "' ");
            }
            int count = -1;
            if (null != obj) {
            ...
            if (count < 1) {
                ...
                sql.append(" Insert into SalesOrderDetailTemp(IndexNo,salesOrderID,GoodsID,ColorID,x_").append(x).append(",Quantity,UnitPrice,DiscountPrice,DiscountRate,RetailSales,Amount,RetailAmount,Discount,BoxQty,SizeStr,SizeIndex,memo) Values(").append(maxIndexNo).append(",'")
                        .append(salesOrderID).append("', '").append(goodsId).append("','").append(colorId).append("' ");
                ...
                // }
                StringBuffer sql = new StringBuffer();
                sql.append(" Update SalesOrderDetailTemp set  UnitPrice =   ").append(UnitPrice).append(", RetailSales = ").append(RetailSales).append(",BoxQty = ").append(boxQty).append(",DiscountPrice = ");
                if (null == DiscountPrice || DiscountPrice.compareTo(BigDecimal.ZERO) == 0) {
                    sql.append(" null ");
                } else {
                ...
                // 备注
                if (meno != null && !"".equals(meno.trim()) && !"null".equalsIgnoreCase(meno)) {
                    sql.append(" , memo = '" + meno + "' ");
                } else {
                    sql.append(" , memo = null ");
                }
                sql.append(", SizeIndex = ").append(sizIndex).append(" where salesOrderID = '").append(salesOrderID).append("' and GoodsID = '").append(goodsId).append("' ");
                if (null != boxQty && 0 != boxQty) {
        ...
        sumSql.append(
                " Update SalesOrder  set QuantitySum = t.QtySum,AmountSum = t.AmountSum, discountRateSum = " + discountRateSum + ", lastARAmount = " + lastARAmount + ", preReceivalAmount = " + preReceivalAmount + ", privilegeAmount = " + privilegeAmount + ",paymentTypeId = " + paymentTypeId + ",  ")
                .append("  RetailAmountSum = t.RetailAmountSum,DiscountSum = t.DiscountSum,displaySizeGroup = ").append(" (SELECT STUFF((select DISTINCT ','''+g.GroupID +'''' from SalesOrderDetailTemp a JOIN goods g ON a.goodsid=g.goodsid ")
                .append(" WHERE salesOrderID='" + salesOrderID + "' FOR XML PATH('')),1,1,''))  ").append(" from  (Select SUM(Quantity) QtySum,Sum(Amount) AmountSum, Sum(RetailAmount) RetailAmountSum,Sum(Discount) DiscountSum  ").append("  from SalesOrderDetailTemp  where SalesOrderID ='")
                .append(salesOrderID).append("'   ) t  ").append(" where SalesOrderID = '").append(salesOrderID).append("' ");
        commonDao.executeSql(sumSql.toString());
        if (preReceivalAmount != null && preReceivalAmount.compareTo(BigDecimal.ZERO) != 0) {
            // 收款金额不为空的时候生成收款单
            String receivalType = "货款";
            BigDecimal arAmount = new BigDecimal(String.valueOf(commonDao.getData(" select AmountSum from salesOrder where salesOrderID = ? ", salesOrderID)));
            generalReceival(deptCode, customerid, departmentid, employeeId, paymentTypeIdStr, arAmount, preReceivalAmount, No, salesOrderID, brandId, receivalType, client);
        }
        if (privilegeAmount != null && privilegeAmount.compareTo(BigDecimal.ZERO) != 0) {
            // 优惠金额不为空的时候生成收款单
            String receivalType = "优惠";
            BigDecimal arAmount = new BigDecimal(String.valueOf(commonDao.getData(" select AmountSum from salesOrder where salesOrderID = ? ", salesOrderID)));
            generalReceival(deptCode, customerid, departmentid, employeeId, paymentTypeIdStr, arAmount, privilegeAmount, No, salesOrderID, brandId, receivalType, client);
        }
        return salesOrderID;
    }

    private int getMaxIndexNo(String salesOrderID) {
        int indexNo = 0;
        StringBuffer maxNoSql = new StringBuffer();
        maxNoSql.append(" select max(IndexNo) IndexNo from StocktakingDetailtemp where StocktakingID =  '").append(salesOrderID).append("'");
    ...
    // 删除发货单数量和价格
    @Override
    public void deleteSalesdetail(List<Map<String, Object>> dataList, String salesOrderID) {
        ...
        for (int i = 0; i < dataList.size(); i++) {
            ...
            if (null != sizeStr && !"".equals(sizeStr) && !"null".equals(sizeStr)) {
                list =
                        commonDao.findForJdbc(" select sdt.salesorderdetailId,isnull(x_" + x + ",0) xCount,isnull(sdt.Quantity,0) counts,isnull(boxQty,1) boxQty from SalesorderDetailTemp sdt with(updlock) "
                                + " join salesorderdetail sd on sdt.salesorderId = sd.salesorderId and sdt.goodsId = sd.goodsId and sdt.colorId = sd.colorId where sdt.salesorderid = '" + salesOrderID + "' and sdt.goodsId = '" + goodsId + "' and sdt.colorId = '" + colorId + "' and sdt.sizeStr = '"
                                + sizeStr + "' ; ");
            } else {
                list =
                        commonDao.findForJdbc(" select sdt.salesorderdetailId,isnull(x_" + x + ",0) xCount,isnull(sdt.Quantity,0) counts,isnull(boxQty,1) boxQty from SalesorderDetailTemp sdt with(updlock) "
                                + " join salesorderdetail sd on sdt.salesorderId = sd.salesorderId and sdt.goodsId = sd.goodsId and sdt.colorId = sd.colorId where sdt.salesorderid = '" + salesOrderID + "' and sdt.goodsId = '" + goodsId + "' and sdt.colorId = '" + colorId + "' ; ");
            }
            if (list.size() > 0) {
                Map dataMap = (Map) list.get(0);
                int xCount = 0, count = 0, boxQty = 1;
                String salesorderdetailId = null;
                if (null != dataMap) {
                    ...
                    salesorderdetailId = String.valueOf(dataMap.get("salesorderdetailId"));
                } else {
                ...
                // !"null".equalsIgnoreCase(oneBoxQtyStr)) {
                // oneBoxQty = Integer.parseInt(oneBoxQtyStr);
                // }
                    // 删除明细
                    // if(count == oneBoxQty){//只有一个尺码
                    sb.append(" delete from SalesOrderDetailTemp where salesOrderdetailId = '").append(salesorderdetailId).append("' ; delete from SalesOrderDetail where GoodsID = '").append(goodsId).append("' and salesOrderID = '").append(salesOrderID)
                            .append("' and ColorID = '" + colorId + "' ; ");
                    // }else{
                    // sb.append(" update SalesOrderDetailTemp set x_"+x+" = null,Quantity = ").append((count-(xCount*boxQty)))
                    ...
                    // .append(",RetailAmount = (RetailSales*(").append(count-(xCount*boxQty)).append(")) where salesorderdetailId = '")
                    // .append(salesorderdetailId).append("' ; update SalesOrderDetail set Quantity = "+(count-(xCount*boxQty))+" where GoodsID = '")
                    // .append(goodsId).append("' and salesOrderID = '").append(salesOrderID).append("' and ColorID = '"+colorId+"' ; ");
                    // }
                } else {// 散件
                        // 删除明细
                    if (count == xCount) {// 只有一个尺码
                        ...
                        sb.append(" update SalesOrderDetailTemp set x_" + x + " = null,Quantity = ").append(count - xCount).append(" ,amount = (isnull(DiscountPrice,UnitPrice)*(").append(count - xCount).append(")),Discount =  (isnull(UnitPrice,0)-isnull(DiscountPrice,0))").append("*")
                                .append(count - xCount).append(",RetailAmount = (RetailSales*(").append(count - xCount).append(")) where salesorderdetailId = '").append(salesorderdetailId).append("' ; update SalesOrderDetail set Quantity = " + (count - xCount) + " where GoodsID = '")
                                .append(goodsId).append("' and salesOrderID = '").append(salesOrderID).append("' and ColorID = '" + colorId + "' and SizeID = '" + sizeId + "' ; ");
                    }
                }
                commonDao.executeSql(sb.toString());
                // 判断明细表是否还有记录
                String sizeStrSum = getSizeStrSum(getMaxSize(goodsId));
                String countStr = String.valueOf(commonDao.getData("select isnull(0" + sizeStrSum + ",0) from SalesOrderDetailTemp where salesorderdetailId = ? ", salesorderdetailId));
                if (countStr != null && !countStr.isEmpty() && !"null".equalsIgnoreCase(countStr) && Integer.parseInt(countStr) == 0) {
                    commonDao.executeSql(" delete from SalesOrderDetailTemp where salesorderdetailId = ? ", salesorderdetailId);
                }
                sb = new StringBuilder();
            }
        }
        // 重新计算货品价格
        sb.append(" update salesorder set DiscountSum =(select sum(Discount) from salesorderdetailtemp where SalesOrderID = '").append(salesOrderID).append("' ) , AmountSum = (select sum(Amount) from salesorderdetailtemp where SalesOrderID = '").append(salesOrderID)
                .append("'), RetailAmountSum = (select sum(RetailAmount) from salesorderdetailtemp where SalesOrderID = '").append(salesOrderID).append("'),QuantitySum = (select sum(Quantity) from salesorderdetailtemp where SalesOrderID = '").append(salesOrderID)
                .append("')  where SalesOrderID = '").append(salesOrderID).append("' ;");
        commonDao.executeSql(sb.toString());
        // 若主表无数据时则删除主表
        List list = commonDao.findForJdbc(" select SalesOrderID from SalesOrder where quantitysum is null ");
        for (int i = 0; i < list.size(); i++) {
            Map map = (Map) list.get(i);
            String salesOrderId = String.valueOf(map.get("SalesOrderID"));
            commonDao.executeSql(" delete from SalesOrder where SalesOrderID = ? ", salesOrderId);
        }
    }
    ...
    public void generalReceival(String deptCode, String customerId, String departmentId, String employeeId, String paymentTypeId, BigDecimal arAmount, BigDecimal receivalAmount, String relationNo, String relationId, String brandId, String type, Client client) throws Exception {
        // 先删除原来关联的收款单
        commonDao.executeSql(" delete from  Receival where receivalId = (select ReceivalID from ReceivalInvoiceDetail where SalesOrderID = ?)  ", relationId);
        // 生成ID
        ...
        sb.append(" Insert Into Receival (ReceivalID,No,Type,Date,CustomerID,DepartmentID,EmployeeID,PaymentTypeID,ReceivalAmount," + "MadeBy,MadeByDate,Audit,AuditDate,AuditFlag,Memo,Year,Month,RelationID,ValidBeginDate,BusinessDeptID,BrandID)"
                + " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)  ");
        commonDao.executeSql(sb.toString(), receivalId, receivalNo, type, DataUtils.str2Timestamp(DataUtils.formatDate()), customerId, departmentId, employeeId, paymentTypeId, receivalAmount, client.getUserName(), DataUtils.gettimestamp(), null, null, 0, "销售订单" + relationNo + "生成",
    ...
    @Override
    public int coverSave(String salesOrderId, List<Map<String, Object>> dataList, Client client) throws Exception {
        int dir = Integer.parseInt(String.valueOf(commonDao.getData(" select QuantitySum from salesOrder where salesOrderId = ? ", salesOrderId)));
        ...
        // 新增或修改
        coverSaveToUpdate(salesOrderId, dir, updateList);
        // 删除
        deleteSalesdetail(delList, salesOrderId);
        // 更新主表
        // 重新计算货品价格
        StringBuffer sb = new StringBuffer();
        sb.append(" update salesOrder set DiscountSum =(select sum(Discount) from salesOrderdetailtemp where salesOrderId = '").append(salesOrderId).append("' ) , AmountSum = (select sum(Amount) from salesOrderdetailtemp where salesOrderId = '").append(salesOrderId)
     ...
     * @param updateList
     */
    private void coverSaveToUpdate(String salesOrderId, int dir, List<Map<String, Object>> updateList) {
        for (int i = 0; i < updateList.size(); i++) {
            ...
            // 判断是否是箱条码
            String typeStr = String.valueOf(commonDao.getData(" select type from salesOrder where salesOrderId = ?", salesOrderId));
            String customerId = String.valueOf(commonDao.getData(" select customerId from salesOrder where salesOrderId = ?", salesOrderId));
            ...
            Object obj = null;
            if (tBoxQty > 0) {
                obj = commonDao.getData(" select count(1) from salesOrderdetailtemp where goodsId = '" + tGoodsId + "' and colorid  = '" + tColorId + "' and salesOrderId = '" + salesOrderId + "' and sizeStr = '" + String.valueOf(temp.get("SizeStr")) + "' ");
            } else {
                obj = commonDao.getData(" select count(1) from salesOrderdetailtemp where goodsId = '" + tGoodsId + "' and colorid  = '" + tColorId + "' and salesOrderId = '" + salesOrderId + "' ");
            }
            int exit = -1;
            if (null != obj) {
            ...
            if (exit < 1) {// 新增
                ...
                maxIndexNo++;
                sql.append(" Insert into salesOrderDetailTemp(IndexNo,salesOrderId,GoodsID,ColorID,x_").append(x).append(",Quantity,UnitPrice,DiscountPrice,DiscountRate,RetailSales,Amount,RetailAmount,Discount,BoxQty,SizeStr,SizeIndex,memo) Values(").append(maxIndexNo).append(",'")
                        .append(salesOrderId).append("', '").append(tGoodsId).append("','").append(tColorId).append("' ");
                if (null == x || "".equals(x) || "null".equalsIgnoreCase(x)) {
                    sql.append(",null ");
                } else {
                ...
                if (Math.abs(tBoxQty) > 0) {
                    ...
                    int ttQty = Integer.parseInt(String.valueOf(commonDao.getData(" select sum(Quantity) from salesOrderDetailTemp where salesOrderId = '" + salesOrderId + "' and goodsId = '" + tGoodsId + "' and colorId = '" + tColorId + "' ")));
                ...
                sql.append(" Update salesOrderDetailTemp set  UnitPrice =   ").append(UnitPrice).append(", RetailSales = ").append(RetailSales).append(",BoxQty = ").append(tBoxQty == 0 ? null : tBoxQty).append(",DiscountPrice = ");
                if (null == DiscountPrice || DiscountPrice.compareTo(BigDecimal.ZERO) == 0) {
                    sql.append(" null ");
                } else {
                    sql.append(DiscountPrice);
                ...
                sql.append(", SizeIndex = ").append(sizIndex).append(" where salesOrderId = '").append(salesOrderId).append("' and GoodsID = '").append(tGoodsId).append("' ");
                if (0 != tBoxQty) {
                    sql.append(" and SizeStr = '").append(sizeStr).append("' ");
                }
	...
	@Override
	public String saveSalesOrderX(List<Map<String, Object>> dataList,
			String salesOrderID, String customerid, String departmentid,
			String employeeId, String businessDeptId, String memo, String type,
			String typeEName, String brandId, String discountRateSum,
			String lastARAmount, String preReceivalAmountStr,
			String privilegeAmount, String paymentTypeId, Client client)
			throws Exception {
		// TODO Auto-generated method stub
		
		
		int direction =1;//销售订单默认是没有这个字段的
        ...
        if ("".equals(salesOrderID)) {
            ...
            // 生成ID
            salesOrderID = commonDao.getNewIDValue(tag);
            // 生成No
            String deptType = client.getDeptType();
            String No = null;
            if (deptType != null && ("直营店".equals(deptType) || "加盟店".equals(deptType))) {
                No = commonDao.getNewNOValue(tag, salesOrderID, client.getDeptCode());
            } else {
                No = commonDao.getNewNOValue(tag, salesOrderID);
            }
            if (salesOrderID == null || No == null) {
                throw new BusinessException("生成主键/单号失败");
            }
            ...
            for(int i=0;i<dataList.size();i++){ //每一条
               ...
               //Discount.add((map.get("Discount")==null || "".equals(map.get("Discount"))) ?new BigDecimal(0):new BigDecimal(String.valueOf(map.get("Discount")))).setScale(2,BigDecimal.ROUND_DOWN);
                ...
                sql="Insert into salesOrderDetailTemp(IndexNo,salesOrderID,GoodsID,ColorID,"+Field+"Quantity,UnitPrice,DiscountRate,DiscountPrice,Discount,Amount,sizeIndex,RetailSales,RetailAmount)"+
            		   "select "+IndexNo+",'"+salesOrderID+"','"+String.valueOf(map.get("GoodsID"))+"','"+String.valueOf(map.get("ColorID"))+"',"
           ...
           System.out.println("尺码组去掉最后一位:"+DisplaySizeGroup);
            ...
            sql ="select isnull(Sum(Quantity),0) Qty,isnull(Sum(Discount),0) DiscountSum,isnull(Sum(Amount),0) Amt,isnull(Sum(RetailAmount),0) RAmt from salesOrderDetailTemp where SalesOrderID= ? ";
	    	    List<Map<String,Object>> ls= commonDao.findForJdbc(sql, salesOrderID);
         ...
         /*
            ...
            */
            ...
            StringBuilder insertMaster = new StringBuilder();
            insertMaster.append(" insert into salesOrder(salesOrderID, No,Date ").append(" , CustomerID,DepartmentID, EmployeeID,  ").append("AmountSum, QuantitySum,  MadeBy, MadeByDate, Type, memo, DiscountSum, Year, Month,  RetailAmountSum, BusinessDeptID, brandId,DisplaySizeGroup  )").append(" values('")
                    .append(salesOrderID).append("', '").append(No).append("', '" + DataUtils.str2Timestamp(DataUtils.formatDate()) + "', '").append(customerid).append("','").append(departmentid).append("', '").append(employeeId).append("', ").append(AmountSum).append(",").append(QuantitySum).append(", '")
        	...
        	//原单记录
        	oldlist =commonDao.findForJdbc("select SalesOrderDetailID,SalesOrderID,GoodsID,ColorID from SalesOrderDetailTemp where SalesOrderID =? ", salesOrderID);
        	 ...
        	 Object obj=null;
	    	  for(int i=0;i<dataList.size();i++){ //每一条
	    		  ...
	    		  String DetailID=	String.valueOf(map2.get("SalesOrderDetailID"));
	    		  ...
	    		  index =getMaxIndexNo(salesOrderID);
	    		  index++;
	    		  obj = commonDao.getData(" select count(1) from salesOrderdetailtemp where goodsId = '" + goodsId + "' and colorid  = '" + colorId + "' and salesOrderID = '" + salesOrderID + "' and salesOrderDetailID= '"+DetailID+"'");
	    	    int count =-1;
	    		  if(obj !=null){
	    			  count =Integer.parseInt(String.valueOf(obj));
	    	      }
          	...
          	String RetailAmount =null;
	    	   ...
	    	   if(count>=1){ //更新存在的一行
	            	if(!"".equals(FieldValue) && FieldValue !=null){
	            		//FieldValue =FieldValue.substring(0, FieldValue.length()-1);
	            		 sql="Update salesOrderdetailtemp set "+UpdateStr+"Quantity="+String.valueOf(Integer.parseInt(String.valueOf(map2.get("Quantity")))*direction)+",UnitPrice="+UnitPrice+",Discount="+dDiscount+",DiscountRate="+DiscountRate+",Amount="+Amount+" where goodsId='"+goodsId+"' and colorid='"+colorId+"' and salesOrderID = '" + salesOrderID + "' and SalesOrderDetailID= '"+DetailID+"'";	
	            		System.out.println("sql语句："+sql);
	    		   ...
	    		   sql="Insert into salesOrderDetailTemp(IndexNo,SalesOrderID,GoodsID,ColorID,"+Field+"Quantity,UnitPrice,DiscountRate,DiscountPrice,Discount,Amount,sizeIndex,RetailSales,RetailAmount)"+
	            		   "select "+index+",'"+salesOrderID+"','"+String.valueOf(map2.get("GoodsID"))+"','"+String.valueOf(map2.get("ColorID"))+"',"
	    	   ...
	    	   sql="select  SalesOrderDetailID from SalesOrderDetailTemp where SalesOrderID='"+salesOrderID+"' and GoodsID ='"+String.valueOf(map2.get("GoodsID"))+"' and ColorID='"+String.valueOf(map2.get("ColorID"))+"'";
               Map<String,Object> nmap=new LinkedHashMap<>();
               nmap.put("SalesOrderDetailID",commonDao.getData(sql)); 
               newlist.add(nmap);
	    	  }//list 结束    	
        	
	    	  for(int i=0;i<newlist.size();i++){
	    		  	 for(int j=0;j<oldlist.size();j++){
	    		    if(newlist.get(i).get("SalesOrderDetailID").equals(oldlist.get(j).get("SalesOrderDetailID"))){//把有的 的的排除掉，因为要保留 删除的,新增的不会在旧的list里面
	    		    	oldlist.remove(j);
	    		    	j--;
	    			  }
	    		  }
	    	  ...
	    	  for(int i=0;i<oldlist.size();i++){
	    		  commonDao.executeSql("delete from SalesOrderDetailTemp where SalesOrderDetailID = '"+String.valueOf(oldlist.get(i).get("SalesOrderDetailID"))+"' and SalesOrderID= '"+salesOrderID+"'");
	    	  }
	    	  
	    	  	 //总和
	    	   sql ="select isnull(Sum(Quantity),0) Qty,isnull(Sum(Amount),0) Amt,isnull(Sum(RetailAmount),0) RAmt from SalesOrderDetailTemp where SalesOrderID= ? ";
	    	   List<Map<String,Object>> ls= commonDao.findForJdbc(sql, salesOrderID);
	    	   sql ="Update SalesOrder set QuantitySum="+String.valueOf(ls.get(0).get("Qty"))+",AmountSum="+String.valueOf(ls.get(0).get("Amt")) +",RetailAmountSum="+String.valueOf(ls.get(0).get("RAmt"))+",displaySizeGroup =(SELECT STUFF((select DISTINCT ','''+g.GroupID +'''' from SalesOrderDetailTemp a JOIN goods g ON a.goodsid=g.goodsid WHERE SalesOrderID='" + salesOrderID + "' FOR XML PATH('')),1,1,'')) where SalesOrderID='"+salesOrderID+"'";
	    	   commonDao.executeSql(sql);
