package com.fuxi.core.common.hibernate.qbc;
...
import com.fuxi.core.common.model.json.DataGrid;
import com.fuxi.system.util.StringUtil;
import com.fuxi.tag.vo.datatable.DataTables;
import com.fuxi.tag.vo.datatable.SortDirection;
import com.fuxi.tag.vo.datatable.SortInfo;
...
public class CriteriaQuery implements Serializable {
     ...
     * 表示法cq.add(cq.or(cq.and(cq, 0, 1), cq, 2))----- hql2:(this_.0 like ? or this_.1 like ?)
     * 表示法:cq.add(cq.or(cq, 0, 1)); 例子：cq.in("TBPrjstatus.code", status); cq.eq("attn",
     * user.getUserName()); cq.isNull("attn"); cq.add(cq.and(cq.or(cq, 1, 2), cq, 0));
     */
     ...
     * @param query
     * @param source
     * @param dest hql:(this_.0 like ? or this_.1 like ?) 表示法:cq.add(cq.or(cq, 0, 1));
     * @return
     */
     ...
     * @param keyname
     * @param keyvalue1
     * @param keyvalue2 两个条件或查询： Restrictions.or(Restrictions.in("username",list1),
     *        Restrictions.idEq(1)); 三个或多个条件查询:（使用嵌套方式） criteria.add(Restrictions
     *        .or(Restrictions.in("username",list1), Restrictions.or(Restrictions.idEq(3),
     *        Restrictions.idEq(4))));
     */
    ...
    public void addSql(String findSql) {
        if (!"".equals(findSql)) {
            criterionList.add(Restrictions.sqlRestriction(findSql));
        }
    }
